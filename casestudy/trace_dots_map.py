...
#             lat, lng, beat = data[8].strip(), data[9].strip(), data[16].strip()
...
#                     print("\t".join([ str(d) for d in (off_id, lat, lng, beat, call_t, disp_t, arv_t, clr_t) ]))
#         except Exception as e:
#             pass

traces = defaultdict(list)
with open("data/patrol.trace.txt", "r") as f:
    for line in f.readlines():
        data   = line.strip().split("\t")
        off_id, lat, lng, beat = data[0], data[1], data[2], data[3]
        call_t, disp_t, arv_t, clr_t = [ float(d) for d in data[4:] ]
        # print(off_id, lat, lng, call_t, disp_t, arv_t, clr_t)
        traces[off_id].append({
            "lat": lat, "lng": lng, "beat": beat,
            "call": call_t, "disp": disp_t, "arv": arv_t, "clr": clr_t,
            "dt": arv_t - disp_t})

for officer in traces:
    n_stops = len(traces[officer])
